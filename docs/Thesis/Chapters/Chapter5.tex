...
\begin{tikzpicture}
    \node [anchor=east] (main) at (-1,5.2) {\Large Main Tabs};
    \node [anchor=east] (cmd) at (-1,4.3) {\Large Design command};
    \node [anchor=east] (toolbox) at (-1,2.5) {\Large Toolbox};
    \node [anchor=east] (name) at (-1,1.5) {\Large Program name};
    \node [anchor=east] (surface) at (-1,0.5) {\Large Design Surface};
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.8\textwidth]{ui_designer.png}};
